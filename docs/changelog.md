...
- 移除「原始订阅类型」概念。转换器本就只按 Clash/Mihomo YAML 解析,`source_type` 从不参与转换
  (surge/loon 是选了即解析失败的假选项),故彻底删除:新建/编辑订阅不再需要选类型;后端删去
  `profiles.source_type` 列(迁移 `0009_drop_source_type.sql`)、相关校验与 API 字段。
- 新建订阅后自动拉取一次:`create` 成功即同步生成一次(复用拉取+转换+缓存),使列表/详情立即反映
  真实拉取状态;尽力而为,失败只记 `last_fetch_status` 不影响创建。随之移除列表的「未拉取」中间态徽章。
- 详情基础信息卡移除「输出类型」展示行(转换输出恒为 mihomo YAML,该字段无实际意义)。
- 移除订阅的「启用/禁用」概念,所有订阅默认且恒为启用。后端删 `profiles.enabled` 列及其索引
  (迁移 `0010_drop_enabled.sql`)、公开端点不再按 enabled 过滤;前端移除详情 hero 状态徽章、
  「启用」展示行与编辑开关、列表「启用中」指标。**注意**:随之失去「暂停订阅(链接返回 404)」
  的能力,停用某订阅只能删除它。
- 规则托管:自定义规则集与机场 `rule-providers` 撞名时不再**静默**覆盖。生成时检测冲突,生成响应
  新增 `ruleset_conflicts` 字段,详情页顶部以告警 banner 提示「以下规则集与机场同名 rule-provider
  冲突,已用面板托管版覆盖」;服务端也记一条 `warn` 日志(覆盖公开/新建路径)。覆盖语义不变——
  管理员显式创建并引用的规则集仍优先,只是不再悄无声息。
- 详情页规则卡移除「添加规则」手动单条新增入口(及其表单弹窗)。规则只能来自「导入机场规则」与
  「导入托管规则」两个导入入口;已有规则仍可逐条编辑/删除/拖拽排序,MATCH 仍钉底。空状态文案改为
  引导用户使用上述两个导入按钮。
- 统一各导航页内容容器尺寸:节点配置、规则托管、系统设置三页的内容宽度与上下边距对齐到订阅配置页
  (`max-width 1180`、`padding 26px 28px 40px`),消除切换导航时内容左右边缘与顶距的跳动。
...
- 「规则托管」全局规则集托管:新增跨订阅复用的全局规则集库,面板把每个规则集托管为永久链接
  ...
  - 前端:新增「规则托管」侧栏入口与页面(规则集卡片列表含远程状态/镜像来源 + 拖拽排序 + 新建/编辑
    弹窗:名称、behavior、format、手动 payload 或 远程 URL/间隔/缓存开关、托管链接预览);从全局规则集库
    引用走「导入托管规则」弹窗——勾选多个托管规则集 + 选引用策略,一次性以 `RULE-SET` 规则引用进当前订阅
    (已引用的标记并禁用)。
...
- 规则编辑 Modal:对全部 Mihomo 规则类型提供贴合类型的 UI 编辑,不再仅有
  DOMAIN-SUFFIX。规则类型选择器改为按分类(域名 / IP / 端口 / 进程 / 入站·其他 /
  逻辑·嵌套 / 兜底)分组的可搜索下拉,并仍允许自由输入未列出的类型;匹配内容输入按类型
  自适应——`NETWORK`(tcp/udp)、`IN-TYPE`(HTTP/SOCKS5/TUN…)给枚举建议,`RULE-SET`
  为规则集名称,`AND`/`OR`/`NOT`/`SUB-RULE` 用多行框并附嵌套语法提示,其余类型按类型
  显示标签与示例占位符。添加与列表内「编辑」共用同一录入弹窗,故两者表现一致。仅前端,
  序列化/解析与后端契约不变。
...
- 配置详情页:将「基础信息」与「原始订阅」两张卡片合并为一张。统一展示
  名称/启用/输出类型 + 原始订阅类型/地址/上次拉取,操作区放「编辑」「更换订阅」
  「立即刷新」三个按钮(两个录入弹窗保留)。仅前端,后端契约不变。
- 托管订阅卡片:把「复制订阅」从 URL 旁的内联小图标改为独立的主按钮(剪贴板 API
  复制,失败有提示)。仅前端。
- 规则编辑 Modal:`no-resolve` 开关对所有规则类型可见(MATCH 除外),不再仅限 IP 类
  规则;移除前端的 `IP_TYPES` 门控。仅前端,序列化/解析逻辑不变。
...
- 简化规则编辑器(`RulesCard`)的主卡片:把单条规则的录入/编辑统一收进 Modal 弹窗
  (顶部「添加规则」按钮新增,行内「编辑」按钮在 Modal 中预填该行),主卡片仅保留
  规则列表 + 兜底策略,列表行保留拖拽排序与删除。移除常驻的添加 composer 行、搜索
  过滤、多选批量(批量删除/批量改策略)与批量编辑 textarea。仅前端改动,规则仍存为
  `rulesets.content` 文本,后端契约不变。

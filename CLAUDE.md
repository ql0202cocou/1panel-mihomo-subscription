...
`web/` SPA); the docs remain the source of truth. The `0.1.7` 1Panel app
package is complete (`apps/mihomo-subscription/0.1.7/`, full install form). To
...
docker build -t mihomo-subscription:0.1.7 .
...
ruby -e 'require "yaml"; ARGV.each { |f| YAML.load_file(f) }' \
  apps/mihomo-subscription/data.yml apps/mihomo-subscription/0.1.7/{data,docker-compose}.yml
```

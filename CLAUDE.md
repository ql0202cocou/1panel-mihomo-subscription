...
docs remain the source of truth. The `0.1.8` 1Panel package is complete
(`apps/mihomo-subscription/0.1.8/`). Ship a new version via `docs/release.md`
...
- Never delete changelog history, user data, or generated app-package files
  ...
  version dir / image tag in sync (current `v0.1.8`).
...
docker build -t mihomo-subscription:0.1.8 .
...
ruby -e 'require "yaml"; ARGV.each { |f| YAML.load_file(f) }' \
  apps/mihomo-subscription/data.yml apps/mihomo-subscription/0.1.8/{data,docker-compose}.yml
```
